...
"@
        ...
        $expected = "`"AppState`"`n{`n`t`"appid`"`t`t`"730`"`n`t" +
            "`"name`"`t`t`"Counter-Strike 2`"`n`t`"SharedDepots`"`n`t{`n" +
            "`t`t`"228989`"`t`t`"228980`"`n`t}`n}`n"
...
Describe "Show-RestartRequired" {
    It "Should call Write-Console and Wait-ForKeyPress" {
        ...
        Should -Invoke Write-Console -Times 1 -ParameterFilter {
