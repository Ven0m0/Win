...
function UUP-Dump-GetISO {
    ...
    function Get-WindowsIso($name, $destinationDirectory) {
        ...
        $uupProcess = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/d', '/c', 'uup_download_windows.cmd') -NoNewWindow -Wait -PassThru
        if ($uupProcess.ExitCode) {
            throw "uup_download_windows.cmd failed with exit code $($uupProcess.ExitCode)"
        }
        Pop-Location
