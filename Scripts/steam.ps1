...
function Start-SteamMin {
    ...
    try {
      ...
      if (-not (Test-Path "$STEAM\steam.exe") -or -not (Test-Path "$STEAM\steamapps\libraryfolders.vdf")) {
        Write-Output "Steam not found!"; Start-Sleep 7; return
      }
    ...
    Get-ChildItem "$STEAM\userdata\*\7\remote\sharedconfig.vdf" -Recurse | ForEach-Object {
      ...
      if ($write) { Set-ContentNoNewline $file (ConvertTo-VDF -Data $vdf); Write-Output "Updated $file" }
    }
...
if ($MyInvocation.InvocationName -ne '.') {
    Start-SteamMin @PSBoundParameters
    exit $LASTEXITCODE
}
