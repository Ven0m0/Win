...
function Write-ColorOutput {
    ...
    .PARAMETER NoNewline
        Switch to write without a newline
    #>
    [CmdletBinding()]
    param(
...
function Show-Menu {
    ...
    for ($i = 0; $i -lt $Options.Count; $i++) {
        Write-ColorOutput "$($i + 1). $($Options[$i])"
    }
    Write-ColorOutput ""
}
...
function Get-MenuChoice {
    ...
    while ($true) {
        ...
        Write-ColorOutput "Invalid input. Please select a valid option ($Min-$Max)." -ForegroundColor Red
    }
}
...
function Show-RestartRequired {
    ...
    Write-ColorOutput $CustomMessage -ForegroundColor Yellow
    Wait-ForKeyPress
}
#endregion
...
function Show-NvidiaGpuSetting {
    ...
    foreach ($item in $settings) {
        Write-ColorOutput "GPU: $($item.GpuName)" -ForegroundColor Cyan
...
function Set-NvidiaSignatureOverride {
    ...
    if (($bcdNoIntegrityExitCode -eq 0) -and ($bcdTestSigningExitCode -eq 0)) {
        ...
        if ($bcdNoIntegrityExitCode -ne 0 -and $bcdNoIntegrityOutput) {
            Write-ColorOutput "    nointegritychecks error (exit code $bcdNoIntegrityExitCode):" -ForegroundColor Yellow
            Write-ColorOutput "      $bcdNoIntegrityOutput"
        }
        if ($bcdTestSigningExitCode -ne 0 -and $bcdTestSigningOutput) {
            Write-ColorOutput "    testsigning error (exit code $bcdTestSigningExitCode):" -ForegroundColor Yellow
            Write-ColorOutput "      $bcdTestSigningOutput"
        }
    }
...
function Set-FullscreenMode {
    ...
    if ($Mode -eq 'FSO') {
        ...
        Write-ColorOutput "Fullscreen Exclusive (FSE) enabled." -ForegroundColor Green
        Write-ColorOutput ""
        Write-ColorOutput "Additional steps may be required:" -ForegroundColor Yellow
        Write-ColorOutput "  1. Right-click game.exe"
        Write-ColorOutput "  2. Select Properties"
        Write-ColorOutput "  3. Go to Compatibility tab"
        Write-ColorOutput "  4. Check 'Disable fullscreen optimizations'"
        Write-ColorOutput "  5. Click Apply"
        Write-ColorOutput ""
        Write-ColorOutput "Note: DX12 engines do not support fullscreen exclusive mode." -ForegroundColor Cyan
    }
}
...
function Set-MultiPlaneOverlay {
    ...
    switch ($Mode) {
        ...
        'Default' {
            ...
            Write-ColorOutput "Multiplane Overlay: Default (Enabled)" -ForegroundColor Cyan
            Write-ColorOutput "Windowed Game Optimizations: Disabled" -ForegroundColor Cyan
        }
    }
}
...
function Show-GamingDisplayStatus {
    ...
    param()
    Clear-Host
    Write-ColorOutput "Current Gaming Display Settings:" -ForegroundColor Cyan
    Write-ColorOutput ""
    ...
    if ($provisioned) {
        foreach ($package in $provisioned) {
            ...
            try {
                ...
                Write-ColorOutput "    Failed to remove provisioned package " `
                    -NoNewline -ForegroundColor Red
                Write-ColorOutput "$($package.DisplayName): $($_.Exception.Message)" `
                    -ForegroundColor Red
            }
        }
    }
...
function Set-EDIDOverride {
    ...
    Write-ColorOutput "Applying EDID Override..." -ForegroundColor Cyan
    Write-ColorOutput ""

    foreach ($monitor in $monitors) {
        $name = $monitor -split '\\'
        Write-ColorOutput "  Applying override for: $($name[1])" -ForegroundColor Green
    ...
    Write-ColorOutput ""
    Write-ColorOutput "EDID override applied successfully to $($monitors.Count) monitor(s)." -ForegroundColor Green
}
...
function Remove-EDIDOverride {
    ...
    Write-ColorOutput "Removing EDID Override..." -ForegroundColor Cyan
    Write-ColorOutput ""

    foreach ($monitor in $monitors) {
        $name = $monitor -split '\\'
        Write-ColorOutput "  Removing override for: $($name[1])" -ForegroundColor Green
        $regPath = "$regLocation$monitor\Device Parameters\EDID_OVERRIDE"
        Remove-RegistryValue -Path $regPath
    }

    Write-ColorOutput ""
    Write-ColorOutput "EDID override removed successfully from $($monitors.Count) monitor(s)." -ForegroundColor Green
}
...
function Show-EDIDStatus {
    ...
    Write-ColorOutput "Current EDID Override Status:" -ForegroundColor Cyan
    Write-ColorOutput ""
    ...
    foreach ($monitor in $monitors) {
        ...
        Write-ColorOutput "Monitor: $($name[1])" -ForegroundColor Yellow
...
function Set-MSIMode {
    ...
    if ($gpuDevices.Count -eq 0) {
        Write-ColorOutput "No display adapters found!" -ForegroundColor Yellow
        return
    }
    ...
    Write-ColorOutput "$status MSI Mode for all GPUs..." -ForegroundColor Cyan
    Write-ColorOutput ""
    ...
    Write-ColorOutput "MSI Mode Status:" -ForegroundColor Cyan
    Write-ColorOutput ""
...
function Write-Header {
    ...
    #>
    param([string]$Text)
    Write-ColorOutput "`n$('='*60)" -ForegroundColor Cyan
    Write-ColorOutput " $Text" -ForegroundColor Cyan
    Write-ColorOutput "$('='*60)`n" -ForegroundColor Cyan
}
...
function Write-Fail {
    ...
    #>
    param([string]$Text)
    Write-ColorOutput "[FAIL] $Text" -ForegroundColor Red
}
...
function Write-Warn {
    ...
    #>
    param([string]$Text)
    Write-ColorOutput "[WARN] $Text" -ForegroundColor Yellow
}
...
function Show-Summary {
    ...
    foreach ($key in $Results.Keys | Sort-Object) {
        ...
        Write-ColorOutput "  $($key.PadRight(25)) : " -NoNewline
        Write-ColorOutput $status -ForegroundColor $color
    }

    Write-ColorOutput "`n  Results: " -NoNewline
    Write-ColorOutput "$successCount succeeded" -ForegroundColor Green -NoNewline
    Write-ColorOutput ", " -NoNewline
    Write-ColorOutput "$failCount failed" -ForegroundColor Red -NoNewline
    Write-ColorOutput ", " -NoNewline
    Write-ColorOutput "$skipCount skipped" -ForegroundColor Yellow
    if ($partialCount -gt 0) {
        Write-ColorOutput ", " -NoNewline
        Write-ColorOutput "$partialCount partial" -ForegroundColor Cyan
    }

    Write-ColorOutput "`n  Duration: $($durationInfo.Duration)" -ForegroundColor Cyan
    Write-ColorOutput "  End Time: $($durationInfo.EndTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Cyan
    Write-ColorOutput ""
}
#endregion
