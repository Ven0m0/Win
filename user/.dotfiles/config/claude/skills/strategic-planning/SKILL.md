...
description: Transforms complex projects into clear, actionable plans through design exploration and strategic breakdown. Guides design validation before implementation, decomposes features into atomic tasks, maps dependencies and critical paths, assesses risks, estimates effort and timeline. Use for new features, refactoring, migrations, team scaling, complex debugging. Provides both design options and implementation roadmaps.
---
...
**CRITICAL: This skill must be manually invoked by the user.** It does not auto-activate. User explicitly chooses when strategic planning (including design exploration) is needed. Always start with design exploration phase before implementation planning.

**HARD-GATE**: Do NOT invoke implementation skills, write code, scaffold projects, or take implementation actions until design has been presented and user approves. This applies regardless of perceived simplicity.
...
Before decomposing into tasks, explore the design space with the user:

**Process**: Check project context → Ask clarifying questions (one at a time) → Propose 2-3 approaches with trade-offs → Present design sections → Document design → Get approval

**Key Principles**: One question per message, multiple choice preferred, remove unnecessary features (YAGNI), explore alternatives before settling, incremental validation, be flexible.

**Output**: Design document saved to `docs/plans/YYYY-MM-DD-<topic>-design.md`, committed to git.

**Anti-Pattern**: "This is too simple to need a design" — every project needs design validation, even simple ones. Design docs can be brief for simple projects, but must be presented and approved.
...
1. **Understand Project Context**: Check existing files, docs, recent commits to ground the conversation
2. **Ask Clarifying Questions**: One at a time, explore purpose, constraints, success criteria
3. **Propose Design Approaches**: Present 2-3 options with trade-offs, lead with your recommendation
4. **Present Design**: Scale to complexity, get approval on each section (architecture, components, data flow, error handling, testing)
5. **Document Design**: Write validated design to `docs/plans/YYYY-MM-DD-<topic>-design.md`, commit to git

**Phase 2: Strategic Breakdown**
6. **Decompose**: Break design into atomic, manageable tasks
7. **Map Dependencies**: Identify blocking and sequencing requirements
8. **Assess Risks**: Identify blockers and uncertainty factors
9. **Sequence & Resource**: Create optimal execution order, estimate effort/timeline
10. **Validate & Adapt**: Review plan for completeness, build flexibility for evolving requirements
