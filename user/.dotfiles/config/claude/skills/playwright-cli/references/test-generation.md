...
End-to-end workflow for authoring and maintaining Playwright tests with `playwright-cli`. Every action emits the equivalent Playwright TypeScript, which is the raw material for every test. The sections below can be used independently:
...
Plan / generate / heal lean on the same mechanic: run `npx playwright test --debug=cli` in the background, then `playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for details.

---

## 0. How generation works

Every `playwright-cli` action generates corresponding Playwright TypeScript code, shown in the output, which you can copy directly into your test files.
...
When asserting text content, make sure the generated locator doesn't contain text from the element itself. `getByTestId()` or `getByLabel()` usually work well for asserting text; for a text-based locator, prefer `toBeVisible()` instead.

The snapshot to be matched doesn't need all the information — only capture what's necessary for the assertion. Use regular expressions for unstable values.
...
Goal: produce a spec file (e.g. `specs/<feature>.plan.md`) enumerating the scenarios to test. **Always** write the spec to a file.
...
A **seed test** is a minimal test that lands the page in the state every scenario starts from: navigation to the app, any required login, feature flags, etc. Scenarios assume a fresh start *after* the seed, and `--debug=cli` pauses *inside* it — so the seed is where every planning and generation session begins.
...
If none exists, create one that at least navigates to the app.
...
**Important**: Don't just open the app URL with playwright-cli — always go through the test to capture any custom setup done there.
...
Goal: turn a spec file into Playwright test files, updating the spec if it has drifted.
...
**Don't** just open the app URL with playwright-cli — always go through the test to capture any custom setup done there.

Walk the scenario's `Steps:` one by one with `playwright-cli`, treating the spec as the plan and the live app as the source of truth. If a step is vague ("click the button" — which button?), references a missing element, or contradicts the app's actual behaviour, use judgement: update the spec to match reality and keep going. Editing the spec mid-generation is expected.
...
Loop 2.2 over the targeted scenarios one at a time, restarting the seed between each so every test starts from a clean page. Safe to parallelize since generated session names are unique — just make sure each test run is stopped.
...
Record the list of failing `<file>:<line>` entries and process them one at a time. Don't attempt parallel fixes — shared state and the single CLI session make that fragile.
...
The test is paused at the start. Step forward or run to just before the failing action or assertion, then diagnose:
...
Rehearse the corrected interaction with `playwright-cli` — the output's generated code is what you paste back into the test.
...
Only after the user answers: update the spec (intentional change) or file/flag the test as covering a bug (regression).

### 3.5 Iteration and giving up

- Fix failures one at a time; rerun after each.
- If, after thorough investigation, you're confident the test is correct but the app is wrong *and* the user confirmed it's a bug: mark the test `test.fixme(...)` with a comment pointing at the user's decision or issue link. Never silently skip.
