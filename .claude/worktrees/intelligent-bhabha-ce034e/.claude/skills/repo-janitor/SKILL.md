...
**6. Clean Documentation Content** — On all remaining documentation markdown files (skip prompt files and agent instruction files): remove emojis, remove "comprehensive", remove hyperbolic/promotional language, apply style conformity. Minimize whitespace: strip trailing spaces on all lines, collapse 2+ consecutive blank lines to one, remove leading/trailing blank lines in each file. Never alter markdown formatting or intentional structure. Badges and graphics are fine; text must be accurate and succinct.
</steps>
