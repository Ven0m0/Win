...
function Action-EnableLargePages {
    Write-Info "Enabling large-page support for PioneerGame.exe..."
    if (-not (Is-Admin)) {
        Write-Err "Administrator required for IFEO registry changes"
        return $false
    }
    $ifeohPath = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\PioneerGame.exe'
    try {
        if (-not (Test-Path $ifeohPath)) {
            New-Item -Path $ifeohPath -Force | Out-Null
        }
        New-ItemProperty -Path $ifeohPath -Name 'UseLargePages' -Value 1 -PropertyType DWord -Force | Out-Null
        Write-Ok "Large-page support enabled for PioneerGame.exe (IFEO)"
        Write-Info "Requires game restart to take effect"
        return $true
    } catch {
        Write-Err "Failed to set IFEO registry key: $_"
        return $false
    }
}
...
while ($true) {
    ...
    Write-Host "     L. $(Checkbox $chkLargePages) Enable large-page support for PioneerGame.exe (IFEO)" -ForegroundColor Gray
    Write-Host ""
    ...
    switch -Regex ($key) {
        ...
        '^L$'  { $chkLargePages = -not $chkLargePages }
        '^[1-5]$' {
            $idx = [int]$key - 1
        ...
        '^$' {
            ...
            if ($chkLargePages) { Write-H "LARGE PAGES";   Action-EnableLargePages | Out-Null }
